...
app.post('/openai/4o', async (c) => {
  try {
    ...
    return c.json({
      ...
      text: `Your question: "${question}" is being processed with GPT-4o. Please wait for the response.`
    })
  } catch (error) {
...
app.post('/openai/o1-pro', async (c) => {
  try {
    ...
    return c.json({
      response_type: "ephemeral",
      text: `Your question: "${question}" is being processed with o1-pro. Please wait for the response.`
    });
  } catch (error) {
